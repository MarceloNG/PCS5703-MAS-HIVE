...
- **KTD2 — Adoção dirigida por obrigação MOISE+ real.** A org emite uma obrigação
  ...
  dono nesta fase)
- **KTD3 — Config de avaliação fiel, em arquivo novo.** Criar um config com os roles
  reais **aditivos** (default restrito; `worker` = default + ações de pontuação,
  **listadas explicitamente** no JSON), `absolutePosition:false`, 70×70. Não usar o
  `standard.json` bundled (sample degenerado — worker sem move) nem poluir a
  `OfficialTestConfig.json` (permissiva: default com tudo). (see origin: R4, KD4)
...
- **Approach:** `absolutePosition:false`, 70×70, `roleZones` presentes. Definir os roles
  da Table 1 **com as ações listadas explicitamente** (o JSON não suporta "default+"):
  `default` = `[skip,move,rotate,adopt,clear,detach]`; `worker` =
  `default + [request,attach,connect,disconnect,submit]` (speed `[2,1,1,0]`, vision 5);
  `constructor`/`explorer`/`digger` definidos para fidelidade mas **não usados** no mínimo
  viável. Manter `conf/FastTestConfig.json` (dev) e `conf/OfficialTestConfig.json`
  intactos.
- **Execution note:** sem código testável; validado pelo boot (U6).
- **Patterns to follow:** estrutura de `conf/OfficialTestConfig.json`; tabela de roles em
  `massim_2022/server/conf/sim/roles/standard.json` (corrigir o defeito: incluir as ações
  de movimento em worker/constructor/explorer).
- **Test scenarios:** `Test expectation: none` — config; comportamento coberto por U6.
- **Verification:** o servidor sobe com o config; um agente `default` recebe `FAILED_ROLE`
  ao tentar `request`, e após `adopt(worker)` numa role-zone passa a poder.
...
- `local/978-3-031-38712-8.pdf` (livro MAPC 2022, Table 1) — roles **aditivos** (worker =
  default + extras); o `standard.json` bundled é sample degenerado.
